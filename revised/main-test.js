...
class structureDepartment {
    ...
    createConstructionSites() {
        for(let y = 0; y < this.roomDepartment.matrixMap.length; y++) {
            for(let x = 0; x < this.roomDepartment.matrixMap[0].length; x++) {
                result = this.roomDepartment.room.createConstructionSite(x, y, this.roomDepartment.matrixMap[y][x]);
                if(result === OK) {
                    this.roomDepartment.matrixMap[y][x] = 'x';
                }
            }
        }
    }
...
var structure = new structureDepartment(Game.spawns['Spawn1'], rooms_man);
