function CreepFactory(spawn) {
    ...
    this.startSpawningCreep = function() { //expects an array, string, and string
        var creep;
        if(_.isEmpty(this.queue)!= true) {
            creep = this.queue.shift();
        } else {
            return undefined;
        }
        let status = this.spawnCreep(creep.name, creep.type, creep.role);
        if(status == true) { //if spawning, set this.spawning to creeper that is spawning
            return 'spawning';

        } else { //if creeper is not spawning, add requested creeper back to queue
            //if(this.queue.find((c) => c.name == name) != true) {
            this.queue.unshift(creep); //put back to beginning of queue
            return undefined;
            //}
        }
    };
